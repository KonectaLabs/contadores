...
| CRM view selector and chat | Stage views are always visible in the queue bar instead of hidden behind `Views`; strategy and tag filters are visible in the same filter board instead of tucked inside a `Filters` button; the message timeline now uses offset inbound/outbound cards, colored rails, and blue/warm role treatment instead of same-looking green/cream blocks. | Filter query behavior, message data, tabs, and send behavior are unchanged. The wider filter board keeps horizontal scrolling as the safe dense layout for small screens. | `npm run build`; Browser local desktop at 1366px with backend data, no horizontal overflow, no `Views` or `Filters` summary buttons, visible `ct-filter-board`, message card rendered with blue outbound treatment; server bundle contains `ct-filter-board`. | Needs a second pass with richer inbound/outbound production conversations after deploy. |
