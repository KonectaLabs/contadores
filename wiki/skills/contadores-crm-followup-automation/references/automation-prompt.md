...
9. If sending live, create a small preview/ledger under data/contadores or data/reports with timestamp, lead ids, copies, queued message ids, and reasons. Use existing backend helpers/actions; avoid raw database mutation except for read-only inspection unless no helper exists.
