...
def test_agent_commands_call_expected_methods_paths_and_bodies(
    ...
    commands = [
        ...
        (
            ["campaigns", "graph", "get", "campaign-1"],
            "GET",
            "/api/agent/campaigns/campaign-1/graph",
            None,
            None,
        ),
        (
            ["campaigns", "graph", "set", "campaign-1", "--graph-json", '{"campaigns":[]}'],
            "PUT",
            "/api/agent/campaigns/campaign-1/graph",
            None,
            {"meta_plan_graph": {"campaigns": []}},
        ),
        (
            [
                "campaigns",
            ...
            "POST",
            "/api/agent/campaigns/campaign-1/graph/duplicate",
            None,
            {
                "node_type": "ad",
                "node_id": "ad-1",
                "target_parent_id": "adset-1",
                "overrides": {"headline": "Variante"},
            },
        ),
        (
            [
            ...
            "POST",
            "/api/agent/campaigns/campaign-1/meta-plan/stage",
            None,
            {"ad_account_id": "act_1", "notes": "", "idempotency_key": "stage-1", "dry_run": True},
        ),
        (
            ["campaigns", "publish", "preflight", "attempt-1", "--run-id", "run-1", "--live-writes-requested"],
            "POST",
            "/api/agent/runs/run-1/tools/preflight_meta_publish_plan",
            None,
            {"arguments": {"attempt_id": "attempt-1", "live_writes_requested": True}},
        ),
        (
            [
                "campaigns",
            ...
            "POST",
            "/api/agent/runs/run-1/tools/approve_meta_publish_plan",
            None,
            {
                "arguments": {
                    ...
                    "require_inventory_ready": True,
                    "max_daily_budget_usd": 50,
                    "max_lifetime_budget_usd": 1500,
                    "max_estimated_monthly_budget_usd": 1500,
                }
            },
        ),
        (
            ["campaigns", "publish", "execute", "attempt-1", "--run-id", "run-1", "--live-writes-requested"],
            "POST",
            "/api/agent/runs/run-1/tools/execute_meta_publish_plan",
            None,
            {"arguments": {"attempt_id": "attempt-1", "live_writes_requested": True}},
        ),
        (
